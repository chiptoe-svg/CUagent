<p align="center">
  <img src="assets/cuagent_2.png" alt="NanoClaw FlexAgents" width="400">
</p>

<p align="center">
  A multi-runtime personal assistant that runs agents securely in containers that fully respects Institution rules and policies. The functionality and skills are built independently from the model provider allowing flexibilty and continuity if/when institution contracts or priorities change (as OpenAI, Anthropic, Google and Local LLM-driven agents are all equally supported).
</p>
...
- **Multi-Runtime Agent Support** — Using Codex (OpenAI) as the primary runtime with Local LLM's, Claude or Gemini available for specific tasks. The ability to switch runtimes per group means privacy-sensitive work could use local models via Ollama, etc. while general tasks use cloud APIs.
